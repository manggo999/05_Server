...
<body>
	...
	<pre>
		...
		<h3> 1. page scope : 현재 페이지</h3>
		-> 현재 JSP에서만 사용 가능한 객체 (Servlet X)
		
		<h3> 2. request(요청) scope == HttpServletRequest</h3>
		-> 요청 받은 페이지(Servlet/JSP) 와 
		   위임 받은 페이지(Servlet/JSP) 에서 유지되는 객체
		   
		<h3> 3. session(입회, 접속) scope </h3>
		...
		- [유지 범위]
		  사이트 접속 ~ 브라우저 종료 or 세션 만료
		
		- session이 유지되는 상태에서는 아무곳에서나 가져다가 사용할 수 있다
		
		<h3> 4. application (ServletContext) scope </h3>
	...
	<ul>
		...
		<li>application scope : ${rapplicationValue}</li>
	</ul>
	
	<a> 객체 범위 확인 페이지로 이동 </a>
